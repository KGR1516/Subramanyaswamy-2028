...
Force one with `LLM_PROVIDER=claude_code|anthropic|openai`. With none of these
available, it uses the built-in **deterministic** rule engine — which needs no
LLM, key, or network and never crashes on missing data. Either way, every
figure an agent cites is verified against the evidence bundle; ungrounded
numbers cause that stock to fall back to deterministic scoring.
